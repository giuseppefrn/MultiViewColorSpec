...
def plot_de(history, output_dir):
    plt.figure(figsize=(16,12))
    plt.plot(history['epoch'], history['validation_DE'], label='val_delta_e', color='orange')
    plt.plot(history['epoch'], history['test_DE'], label='test_delta_e', color='red')
    plt.plot(history['epoch'], history['DE'], label='delta_e', color='blue')
    plt.legend()
    plt.title('DELTA E')
    check_dir(output_dir)
